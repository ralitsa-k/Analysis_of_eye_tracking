...
for (i in 3:37){
  ...
  gaze_data[[(i - 2)]] <- read_csv(paste0('Y:/TOOLS/runEYETRACKING/analyse_eyetracking/Analysis_of_eye_tracking/data/',
                                                    subj_id[[i-2]], '_selection_gaze.csv')) %>%
...
big_gaze_data1 <- big_data1 %>%
  ...
  mutate(ngaze = n()) %>%
  mutate(idx_ns = seq(1:ngaze)) %>%  # Таке the second half (time) of each trial 
...
behav_data <- read_csv('Y:/TOOLS/runEYETRACKING/analyse_eyetracking/Analysis_of_eye_tracking/behav_data_combined.csv') %>%
  ...
  select(ID, participant, block_soc, trial_id, left_rew_dbl, right_rew_dbl, trial_n, trigger_integer) 
...
behav_and_eye <- behav_data %>%
  ...
  separate(trigger_integer, into = c('stim1', 'stim2'), -1, remove = FALSE) %>%
  mutate(stim1 = as.numeric(stim1),
         stim2 = as.numeric(stim2),
         eye_choice = as.numeric(eye_choice))
...
write.csv(eye_pref_cleaned, 'eye_pref_cleaned_selection.csv')
...
data_2b <- read_csv('eye_pref_cleaned_selection.csv')
...
bsl %>%
  ...
  scale_colour_manual(values = c(color_both_high, color_both_low)) +
...
# Full experiment learning curve -----------------

data_tofill = tibble(ID = rep(c(dd$ID), each = 540), 
                     trial = rep(1:180,times = sjs))
outgr <- data_2b %>%
  filter(block_soc == 'out-group') %>%
  full_join(data_tofill) %>%
  arrange(ID, trial_n) %>%
...
outgr_behav = fix_big_data %>%
  filter(block_soc == 'out-group') %>%
  transmute(behav_response = response, participant, trial_id) %>%
  inner_join(outgr) %>%
  mutate(match = ifelse(behav_response == eye_choice,1, 0))



# Overall risk-taking --------------------------------------
# No difference between social conditions 

risky_eyes = data_2b %>%
  mutate(risky = ifelse(trigger_integer %in% c('12','21') & stim_chosen == 1, 1,
                        ifelse(trigger_integer %in% c('34','43') & stim_chosen == 3, 1, 
                               ifelse(trigger_integer %in% c('14', '41') & stim_chosen == 1, 1,
                                      ifelse(trigger_integer %in% c('23', '32') & stim_chosen == 3, 1, 0)))),
         riskChosen = ifelse(trigger_integer %in% c('13', '31', '24', '42'), NA, risky),
         risky = riskChosen)
  
dddd = risky_eyes %>%
  filter(trial_id > 120) %>%
  group_by(ID,block_soc,pair_type) %>%
  summarise(mean_risk = mean(risky,na.rm = TRUE)) 
ggplot(dddd, aes(x = block_soc, y = mean_risk, fill = pair_type)) +
  geom_boxplot()
library(lme4)
summary(glmer(data = risky_eyes, risky ~ block_soc*pair_type + (1|ID), family = 'binomial'))
